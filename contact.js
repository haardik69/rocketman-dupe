...
let iapToken = "";
    ...
    const iapClient = google.accounts.oauth2.initTokenClient({
        client_id: "536964574187-51j77krhjata7cbl8l23d9cb0hqtbnr4.apps.googleusercontent.com",
        scope: "",
        callback: (resp) => {
        iapToken = resp.access_token;
        document.getElementById("contact-form").dispatchEvent(new Event("iap_ready"));
        }
    });
    ...
    document.getElementById("contact-form")
        ...
        e.preventDefault();
        iapClient.requestAccessToken();
        });

    document.getElementById("contact-form")
        .addEventListener("iap_ready", function() {
        const form = this;
        const data = {
        ...
        .then(r => r.json())
        .then(resp => {
            if (resp.status === "success") showPopup("success");
            else                       showPopup("fail");
        })
        .catch(() => showPopup("fail"));
        });